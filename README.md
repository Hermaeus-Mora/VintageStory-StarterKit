...
## Конфигурация
**ResetOnDeath** – (System.Boolean) указывает, нужно ли сбрасывать счётчик полученных наборов после смерти;\
...
Пример конфигурации (каждый игрок может получить 2 стартовых набора, состоящих из одной темпоральной шестерни, 16 хвощевых припарок и грубый лук только для охотника):
<details>
...
**ForClass** – (System.String) класс, для которого предназначен стак предметов. Если не указан, то стак получит любой класс;\
**Attributes** – (StackAttribute[]) атрибуты стака предметов.

<details>
  <summary>Стандартные классы</summary>
